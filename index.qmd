...
# Pull all artefacts from the {targets} pipeline. Run
#   LC_ALL=en_US.UTF-8 LANG=en_US.UTF-8 Rscript -e 'targets::tar_make()'
# (or rely on the report target in CI) before rendering this report.
updated                    <- tar_read(updated_string)
date_recent                <- tar_read(date_recent_string)
...
@fig-data-openness-wo-nd zeigt, wie einfach und offen der Zugriff auf Daten-Downloads der einzelnen Kantone auf geodienste.ch ist. Das Vorgehen zur Bewertung der Offenheit des Zugriffs ist unter @sec-bewertung-der-offenheit erklärt. Die Abbildung zeigt die relativen Anteile der Zugriffskategorien für alle Geodaten des jeweiligen Kantons, für die nicht ausdrücklich «Keine Daten» bzw. «keine Daten / Bereitstellung» ausgewiesen wird (siehe dazu auch die Ausführungen in @sec-vollstaendigkeit).
...
#| fig-cap: !expr str_c("Verfügbarkeit (Zugriffskategorie) von Geodaten auf geodienste.ch  (Stand ", updated, "). Kategorien ohne Daten («Keine Daten» und «keine Daten / Bereitstellung») sind ausgeblendet. Die Datensatz-Bezeichnungen, die beim Überfahren der Grafik erscheinen, sind in @sec-datensaetze erklärt.")
...
Auch in @fig-data-openness-wo-nduc wird die Offenheit des Zugriffs auf Daten-Downloads der einzelnen Kantone auf geodienste.ch dargestellt. Im Unterschied zur @fig-data-openness-wo-nd sind hier jedoch nur die Daten enthalten, die grundsätzlich verfügbar sind. Falls in einem Kanton für ein Thema die Daten noch «Im Aufbau» sind bzw. falls für ein Thema «Keine Daten» oder «keine Daten / Bereitstellung» ausgewiesen wird, dann fliesst diese Information nicht in die Darstellung ein. Bewertet wird hier also nur die Zugänglichkeit der tatsächlich vorhandenen Daten.

```{r}
#| label: fig-data-openness-wo-nduc
#| fig-cap: !expr str_c("Verfügbarkeit (Zugriffskategorie) von Geodaten auf geodienste.ch (Stand ", updated, "). Kategorien ohne Daten («Keine Daten» und «keine Daten / Bereitstellung») sowie «Im Aufbau» sind ausgeblendet. Die Datensatz-Bezeichnungen, die beim Überfahren der Grafik erscheinen, sind in @sec-datensaetze erklärt.")
#| column: page-inset-right
#| fig-height: 10
#| fig-width: 10
#| warning: false
...
@fig-data-completeness zeigt, für wie viele Kategorien in den einzelnen Kantonen auf geodienste.ch keine Daten angeboten werden oder die Daten noch im Aufbau sind. Laut Auskunft von verschiedenen Geoinformationsfachstellen können Geodaten aus zumindest zwei Gründen nicht verfügbar sein (und werden daher je nach Datengrundlage mit «Keine Daten» oder «keine Daten / Bereitstellung» gekennzeichnet):
...
-   *Keine Daten* bzw. *keine Daten / Bereitstellung*: Es liegen zum jeweiligen Thema im jeweiligen Kanton keine Geodaten vor bzw. sie werden nicht bereitgestellt.
...
-   *keine Daten / Bereitstellung*: 1.5 Punkte, da die fehlende Bereitstellung nicht durch den Kanton verantwortet wird
...
| Abkürzung | Datensatz |
|-------------------|-----------------------------------------------------|
| ARAF | Abwasserreinigungsanlagen (ARA) mit Finanzkennzahlen |
| ARA | Abwasserreinigungsanlagen (ARA) ohne Finanzkennzahlen |
...
| GEPF | Kommunale Entwässerungsplanung (GEP) mit Finanzkennzahlen |
| GEP | Kommunale Entwässerungsplanung (GEP) ohne Finanzkennzahlen |
| Gk | Gefahrenkarten |
| Gwr | Gewässerraum |
...
| PET | Kantonale Klimakarte – Physiologisch äquivalente Temperatur (PET) |
...
| RlA | Rohrleitungsanlagen |
| RPk | Richtpläne der Kantone |
...
| VtN | Versorgungsgebiete thermische Netze |
