...
::: callout-tip
Bei der Gestaltung dieser Webseite wurde darauf geachtet, dass die Informationen für möglichst viele Personen verständlich sind. Spezielle Begriffe sind mit einem Kommentar erklärt. Wenn Sie auf die hochgestellte Zahl klicken, erhalten Sie in einer Fussnote eine kurze Erklärung des vorangestellten Begriffs. Hier ist ein Beispiel^[Hier ist ein Beispiel eines erläuternden Kommentars. Mit Klick auf das Symbol &laquo;↩︎&raquo; gelangen Sie übrigens an genau den Ort zurück, von dem Sie zu dieser Erklärung gesprungen sind: ].
:::

Unter [geodienste.ch](https://geodienste.ch) stellen die Kantone und das Fürstentum Liechtenstein ihre sogenannten Geobasisdaten^[Geobasisdaten 
sind eine spezielle Art von Geodaten, nämlich «Geodaten, die auf einem rechtsetzenden Erlass des Bundes, eines Kantons oder einer Gemeinde beruhen» (siehe [Artikel 3 des Bundesgesetzes über Geoinformation, Fedlex 510.62](https://www.fedlex.admin.ch/eli/cc/2008/388/de#art_3))] zur Verfügung. Die Daten sind so für interessierte Personen und Organisationen zugänglich.

Die Geobasisdaten sind möglichst schweizweit verfügbar, strukturell harmonisiert^[das heisst: in pro Thema einheitlichen Datenmodellen] und aggregiert^[das heisst: über Kantonsgrenzen hinweg zusammengefügt]. Sie können von geodienste.ch auf verschiedenen Wegen bezogen und gemäss der jeweiligen Nutzungsbedingungen für eigene Zwecke genutzt werden.
...
-   **Daten-Download** bezeichnet in der vorliegenden Analyse das Herunterladen von Daten in Form von Dateien oder die Nutzung eines sogenannten Downloaddienstes. geodienste.ch bietet dateibasierte Daten-Downloads in verschiedenen gängigen Datenformaten^[aktuell: Interlis, Shapefile und Geopackage] an.
...
::: callout-note
Sämtlicher Programm-Code hinter dieser Webseite -- inklusive Bezug, Aufbereitung und Analyse der Daten von geodienste.ch -- ist [auf GitHub einsehbar](https://github.com/rastrau/geodienste-ch).

Haben Sie einen Fehler entdeckt? Bitte geben Sie mir einen [Hinweis per Mail](mailto:geodienste-ch@ralphstraumann.ch) oder [eröffnen Sie einen Issue auf GitHub](https://github.com/rastrau/geodienste-ch/issues). Vielen Dank!
:::
...
-   *Frei verfügbar*: Dies ist die offenste Zugriffskategorie. Geodaten in dieser Kategorie können, müssen aber nicht^[denn: Manche Kantone schränken die Nutzungsrechte auch von frei verfügbaren Geodaten ein], offene Verwaltungsdaten bzw. Open Government Data (OGD)^[das heisst: Jede Person und Organisation darf frei darauf zugreifen, sie nutzen, verändern und teilen &ndash; eingeschränkt höchstens durch Massnahmen, die Ursprung und Offenheit der Daten bewahren] sein.
...
-   *«Ich möchte mit den Daten arbeiten»*: Manche Nutzerinnen und Nutzer möchten Geodaten nicht nur ansehen, sondern sie selbst auswerten, verändern oder weiterverarbeiten -- beispielsweise mit einem GIS^[Geographisches Informationssystem: eine Software zur Erstellung, Pflege und Analyse von räumlichen Daten], mit einem CAD-Programm^[Computer-Aided Design-Programm: eine Software zur Erstellung von technischen Zeichnungen] oder mit einer Statistik-Software.
...
Neben dem Download von Geodaten von geodienste.ch in Form einer Datei ist auch die Nutzung von sogenannten WFS^[Abkürzung für «Web Feature Service», ein Standard des Open Geospatial Consortium OGC] möglich. Die Geodaten aus dem WFS können entweder dynamisch genutzt^[Hier ist gemeint: Die Daten werden zum Nutzungszeitpunkt vom WFS bezogen. Die Nutzerin oder der Nutzer verwendet somit immer denjenigen Stand der Daten, den der Betreiber zum aktuellen Zeitpunkt dem WFS hinterlegt hat. Somit können sich die Daten zwischen zwei Nutzungszeitpunkten verändern. Dies ist der Nutzerin oder dem Nutzer in der Regel aber nicht bewusst und ist oft nicht trivial herauszufinden.] werden oder durch die Nutzerin oder den Nutzer in einer Datei dauerhaft gespeichert werden.

### Angaben für technisch Interessierte

Als Datenquelle für diese Webseite wird die auf [Swagger](https://swagger.io) und der [OpenAPI Specification 3](https://spec.openapis.org/oas/latest.html) basierende Programmierschnittstelle (API^[Application Programming Interface]) von geodienste.ch genutzt. Via den `services`-Endpoint des API werden mit dem folgenden API-Call die zur Verfügung stehenden Daten und Darstellungsdienste im JSON-Format^[Javascript Object Notation-Format] abgefragt: `https://www.geodienste.ch/info/services.json?restricted_topics=false&language=de`.
...
Die Workflows für den Bezug der Daten und für die Analyse selbst werden mittels [GitHub Actions](https://github.com/features/actions) automatisch umgesetzt. Das Dependency Management^[Verwaltung der für die erfolgreiche Ausführung eines Programms notwendigen Ressourcen wie beispielsweise Libraries oder Module] erfolgt mit [pip](https://pypi.org/project/pip) und einer `requirements.txt`-Datei für die Python-Komponente bzw. mit [renv](https://rstudio.github.io/renv/articles/renv.html) für die R-Komponente.
